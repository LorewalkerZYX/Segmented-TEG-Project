...
# Preparing the data
[trainx, trainy, validx, validy, test_x, test_y] = LoadDataQR1_.LoadData()

testing_x = test_x.copy()
testing_y = test_y.copy()

# normalization
trainx = NormalQR1_.normalize(trainx)
validx = NormalQR1_.normalize(validx)
testx = NormalQR1_.normalize(test_x)
trainy = NormalQR1_.normalize(trainy, False)
validy = NormalQR1_.normalize(validy, False)
Y_test = NormalQR1_.normalize(test_y, False)
...
# save in excel
workbook = xlsxwriter.Workbook('NewQin_train_result_R1\'.xlsx')
...
Predict_y = NormalQR1_.recover(t_out)
...
# print(temp1)
workbook.close()
torch.save(net.state_dict(), 'NewTEGNetSegQR1\'.pkl')
